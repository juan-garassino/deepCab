...
def get_pandas_chunk(
    path: str, index: int, chunk_size: int, dtypes, columns: list = None, verbose=True
) -> pd.DataFrame:
    """
    ...
    if verbose:
        print(
            Fore.MAGENTA
            + f"Source data from {path}: {chunk_size if chunk_size is not None else 'all'} rows (from row {index})"
            + Style.RESET_ALL
        )
...
def save_local_chunk(path: str, data: pd.DataFrame, is_first: bool):
    """
    ...
    data.to_csv(path, mode="w" if is_first else "a", header=is_first, index=False)
